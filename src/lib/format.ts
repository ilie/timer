...
import { MINUTES_PER_HOUR, MS_PER_MINUTE, MS_PER_SECOND, SECONDS_PER_HOUR, SECONDS_PER_MINUTE } from './time';
...
const estimatedWidth = (segments: readonly RemainingSegment[]): number =>
    segments.reduce(
        (total, segment) => total + segment.text.length * (segment.scale === 'full' ? 1 : SMALL_SCALE_WIDTH),
        0,
    );
...
export const widestRemainingString = (): string => joinSegments(widestRemainingSegments(MAX_REMAINING_MS));
