...
import { displayedRemainingMilliseconds, thresholdOf } from '../lib/timer';
import type { TimerState } from '../lib/timer';

type ValueAlignment = 'start' | 'center';
...
export function RemainingTime({
    ...
    const now = useNow();
    const remaining = displayedRemainingMilliseconds(timer, durationMilliseconds, now);
    ...
    const reservations = useMemo(() => reservationSegments(durationMilliseconds), [durationMilliseconds]);
        ...
        <BoardValue label={label} className={className}>
            ...
            >
                ...
                <span
                    role="timer"
                    aria-live="off"
                    className="col-start-1 row-start-1 font-semibold whitespace-nowrap group-data-[state=critical]/countdown:font-bold group-data-[state=zero]/countdown:font-bold"
                >
