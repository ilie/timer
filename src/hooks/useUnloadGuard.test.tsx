...
const callsFor = (spy: ListenerSpy): number =>
  spy.mock.calls.filter((call) => call[0] === "beforeunload").length;
...
describe("unload guard", () => {
  it("guards only while a session is running and lets go when its end time passes", () => {
    ...
    render(
      <Board onAddSession={noRequest} onEditSession={noRequest} onEditCentreNumber={noRequest} />,
    );
