...
const storedBoard = (sessions: readonly unknown[], breakTimer: unknown = { status: 'idle' }): string =>
    JSON.stringify({
        centreNumber: 'ES432',
        sessions,
...
describe('session lifecycle', () => {
    ...
    it('adds sessions up to the cap with distinct ids and the chosen configuration', () => {
        ...
        expect(getSnapshot().sessions.map((session) => session.examName)).toEqual(['B2 First', 'Linguaskill General']);
