...
const choosePart = async (user: UserEvent, partName: string) => {
    await user.selectOptions(screen.getByLabelText('Component'), screen.getByRole('option', { name: partName }));
};
...
test('persists an edited centre number and leaves it alone when cancelled', async () => {
    ...
    expect(getSnapshot().centreNumber).toBe('ES777');
    expect(screen.getByRole('button', { name: /^Edit centre no:/ })).toHaveTextContent('Centre no: ES777');
});
