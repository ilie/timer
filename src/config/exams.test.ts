...
describe('exams', () => {
    ...
    it('qualifies every Linguaskill part by the Linguaskill rule', () => {
        ...
        const linguaskillExams = allExams.filter((exam) => exam.examName.startsWith('Linguaskill'));
...
describe('exam data every countdown depends on', () => {
    it('gives every component a whole, positive number of minutes', () => {
        for (const part of allParts) {
            expect(Number.isInteger(part.minutes), `${part.id} minutes must be a whole number`).toBe(true);
    ...
    it('keeps every component within a plausible exam length', () => {
        for (const part of allParts) {
            expect(part.minutes, `${part.id} is longer than any Cambridge paper`).toBeLessThanOrEqual(240);
        }
    });
